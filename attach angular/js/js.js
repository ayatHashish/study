...
function openModal(file, index = -1, isMain = false) {
  ...
  // إخفاء أزرار التدوير
  document.getElementById("rotateImageBtn").style.display = "none";
  document.getElementById("rotatePdfBtn").style.display = "none";
  ...
  if (file.type === "application/pdf") {
    ...
    // إظهار زر تدوير PDF فقط
    document.getElementById("rotatePdfBtn").style.display = "inline-block";
    ...
    // إظهار زر تدوير الصورة فقط
    document.getElementById("rotateImageBtn").style.display = "inline-block";
  }
}
function closeModal() {
...
function renderPage(num) {
  pdfDoc.getPage(num).then(function (page) {
    let viewport = page.getViewport({ scale: scale, rotation: pdfRotation });
    pdfCanvas.height = viewport.height;
    pdfCanvas.width = viewport.width;
    let renderContext = { canvasContext: ctx, viewport: viewport };
    page.render(renderContext);
